...
namespace GameOfLife
{
    class LifeSimulation
    {
        private int _heigth;
        private int _width;
        private int numOfAliveNeighborsEmpty;
        private bool[,] cells;
        ...
        public LifeSimulation(int Heigth, int Width)
        {
            ...
            emptyCell = new bool[Heigth, Width];
            GenerateField();
        }
        ...
        private int EmptyCells(int x, int y)
        {
            Random rand = new Random();
            
            int num;
            for (int i = 0; i < emptyCell.GetLength(0); i++)
            {
                for (int j = 0; j < emptyCell.GetLength(1); j++)
                {
                    num = rand.Next(5);
                    emptyCell[i, j] = (num == 0) ? false : true;

                    if (num <= 2)
                    {
                        numOfAliveNeighborsEmpty++;
                    }
                }
            }
            return numOfAliveNeighborsEmpty;
        }
